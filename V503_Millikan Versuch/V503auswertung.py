...
if((os.path.exists('data\daten_tröpfchen_liste.txt'))-1):
    ...
    U, t_0, T = np.loadtxt('V503.txt', unpack = True) # [V],[s],[K]
    v_0 = s/t_0

    teilchen_liste = open('data\daten_tröpfchen_liste.txt', 'w')
    teilchen_liste.write('# Nr    U    v_auf    dv_auf    v_ab    dv_ab    v_0    T    OK\n')

    for i in range(0,Tröpfchen):
        t_auf, t_ab = np.loadtxt('data\daten_tröpfchen_{}.txt'.format(i+1), unpack = True) # [s],[s]
        n = len(t_auf)
    ...
    # Daten laden
    Nr, U, v_auf, Δv_auf, v_ab, Δv_ab, v_0, T, OK = np.loadtxt('data\daten_tröpfchen_liste.txt', unpack = True)
...
######### Debug-Ausgabe ##############
    ...
    x = np.linspace(1.45e-19,1.8e-19,16000)
    y = np.zeros(16000)
